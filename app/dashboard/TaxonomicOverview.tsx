...
export default function TaxonomicOverview({
    ...
    const drawLabels = (labels: any[]) => {
      labels.forEach((labelData: any) => {
        ...
        if (name.length > maxCharsPerLine) {
          ...
          text.append("tspan").text(line2).attr("x", textX).attr("dy", "1.1em");
        } else {
          text.append("tspan").text(name).attr("x", textX).attr("dy", "0");
        }
  ...
  }, [isClient, analysisData.totalSequences, isDark, taxonomicData]);
