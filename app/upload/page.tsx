...
const ALLOWED_FILE_TYPES = [
  ...
  "image/png",
  "image/gif",
  "text/x-fastq", // Added .fastq mime type
];
...
// Theme colors
const THEME_PRIMARY = "#7A7FEE"; // main accent color
const THEME_PRIMARY_LIGHT = "#7A7FEE33"; // hover/alert BG
const THEME_PRIMARY_DARK = "#6366f1"; // active button
const THEME_TEXT = "#111111";
const THEME_TEXT_DARK = "#FFFFFF";
...
export default function UploadPage() {
  ...
  const validateFile = useCallback((file: File): string | null => {
    ...
    if (!ALLOWED_FILE_TYPES.includes(file.type)) {
      return "Please upload a valid file type (PDF, DOC, DOCX, TXT, FASTA, FASTQ, or image files)";
    }
    return null;
  }, []);
  ...
  const handleContinue = useCallback(() => router.push("/processing"), [router]);
  ...
  const FileIcon = React.memo(() => (
    <svg className="w-8 h-8 text-[#7A7FEE]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      <path
        strokeLinecap="round"
  ...
  const DocumentIcon = React.memo(() => (
    <svg className="w-6 h-6 text-[#7A7FEE]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      <path
        strokeLinecap="round"
        strokeLinejoin="round"
        strokeWidth={2}
  ...
  const CheckIcon = React.memo(() => (
    <svg className="w-5 h-5 text-[#7A7FEE] mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="container mx-auto px-4 py-8 pt-20 sm:pt-24 md:pt-28 lg:pt-32">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="text-center mb-6 sm:mb-8">
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-[#111111] dark:text-[#FFFFFF] mb-3 sm:mb-4">
            ...
            {!selectedFile ? (
              <div
                className={`border-2 border-dashed rounded-xl p-8 sm:p-12 text-center transition-all duration-200 cursor-pointer ${
                  dragActive
                    ? "border-[#7A7FEE] bg-[#7A7FEE33]"
                    : "border-gray-300 dark:border-gray-600 hover:border-[#7A7FEE] hover:bg-[#7A7FEE33]"
                }`}
              ...
              >
                <div className="space-y-3 sm:space-y-4">
                  <div className="mx-auto w-12 h-12 sm:w-16 sm:h-16 bg-[#7A7FEE1A] rounded-full flex items-center justify-center">
                    <FileIcon />
                  </div>
                  <div>
                  ...
                  <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 px-2">
                    Supports FASTA, FASTQ, CSV, Excel & image files up to 10MB
                  </p>
                </div>
                <input
                  ref={fileInputRef}
                  ...
                  className="hidden"
                  accept=".pdf,.doc,.docx,.txt,.fasta,.fastq,.jpg,.jpeg,.png,.gif"
                />
              </div>
            ) : (
              <div className="space-y-4 sm:space-y-6">
                ...
                <div className="bg-white dark:bg-[#111111] rounded-xl p-4 sm:p-6 border border-gray-200 dark:border-gray-700">
                  <div className="flex items-center space-x-3 sm:space-x-4">
                    <div className="w-10 h-10 sm:w-12 sm:h-12 bg-[#7A7FEE1A] rounded-lg flex items-center justify-center flex-shrink-0">
                      <DocumentIcon />
                    </div>
          ...
          {/* Success Message */}
          {uploadProgress === 100 && (
            <div className="mt-4 sm:mt-6 bg-[#7A7FEE33] border border-[#7A7FEE] rounded-xl p-4 mx-4 sm:mx-0">
              <div className="flex items-center">
                <CheckIcon />
                <p className="text-[#7A7FEE] font-medium text-sm sm:text-base">
